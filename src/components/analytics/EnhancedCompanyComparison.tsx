...
import { X, TrendingUp, TrendingDown, AlertTriangle, Download } from 'lucide-react';
...
interface EnhancedCompanyComparisonProps {
  companies: Company[];
  onRemoveCompany: (companyId: string) => void;
}

const EnhancedCompanyComparison: React.FC<EnhancedCompanyComparisonProps> = ({ companies, onRemoveCompany }) => {
  ...
  const getRiskLevel = (direction: string, overallScore: number) => {
    if (direction === 'positive' && overallScore >= 70) return { level: 'Low Risk', color: 'text-green-600' };
    if (direction === 'negative' || overallScore < 40) return { level: 'High Risk', color: 'text-red-600' };
  ...
  const industryAverages = companies.length > 0 ? {
    overall: Math.round(companies.reduce((sum, c) => sum + c.overallScore, 0) / companies.length),
    focus: Math.round(companies.reduce((sum, c) => sum + c.focusScore, 0) / companies.length),
    environment: Math.round(companies.reduce((sum, c) => sum + c.environmentScore, 0) / companies.length),
    claims: Math.round(companies.reduce((sum, c) => sum + c.claimsScore, 0) / companies.length),
    actions: Math.round(companies.reduce((sum, c) => sum + c.actionsScore, 0) / companies.length)
  } : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Company Comparison Report</h2>
        <ExportCapabilities selectedCompany={companies[0]} companies={companies} />
      </div>

      {/* Company Cards with Enhanced Info */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {companies.map((company) => {
          const risk = getRiskLevel(company.netActionDirection, company.overallScore);
          return (
                  ...
                  <div className="space-y-2 pt-2 border-t">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm capitalize">{company.netActionDirection}</span>
                      </div>
      ...
      {/* Enhanced Score Comparison Chart */}
      <Card>
        <CardHeader>
          <CardTitle>Detailed Score Comparison</CardTitle>
          {industryAverages && (
            <p className="text-sm text-gray-600">
              Group average: Overall {industryAverages.overall}/100
            </p>
          )}
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Risk Assessment & Investment Insights</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-center py-3">Investment Signal</th>
                </tr>
              </thead>
              <tbody>
                {companies.map((company) => {
                  const risk = getRiskLevel(company.netActionDirection, company.overallScore);
                  const investmentSignal = company.overallScore >= 70 && company.netActionDirection === 'positive' ? 'Strong Buy' :
                                         company.overallScore >= 50 ? 'Hold' : 'Caution';
                  const signalColor = investmentSignal === 'Strong Buy' ? 'text-green-600' :
                                    investmentSignal === 'Hold' ? 'text-yellow-600' : 'text-red-600';
...
export default EnhancedCompanyComparison;
